...
import se.digg.wallet.feature.enrollment.EnrollmentGraphScopedViewModel
import se.digg.wallet.feature.enrollment.OnboardingEvent
import se.digg.wallet.feature.enrollment.activation.ActivationScreen
...
import se.digg.wallet.feature.enrollment.pid.PidScreen
    ...
    NavHost(
        ...
        navigation(
            ...
            composable(EnrollmentNavItem.Intro.route) {
                val sharedViewModel = EnrollmentGraphScopedViewModel(
                    navController = navController,
                    graphRoute = RootGraph.Enrollment
                )
                IntroScreen(navController = navController, onContinue = {
                    navController.navigate(EnrollmentNavItem.ContactInfo.route)
                })
            }
            composable(EnrollmentNavItem.Terms.route) {
                val sharedViewModel = EnrollmentGraphScopedViewModel(
                    navController = navController,
                    graphRoute = RootGraph.Enrollment
                )
            ...
            composable(EnrollmentNavItem.ContactInfo.route) {
                val sharedViewModel = EnrollmentGraphScopedViewModel(
                    navController = navController,
                    graphRoute = RootGraph.Enrollment
                )
                ContactInfoScreen(navController = navController, onContinue = {
                    navController.navigate(EnrollmentNavItem.Pin.route)
                })
            }
            composable(EnrollmentNavItem.Pin.route) {
                val sharedViewModel = EnrollmentGraphScopedViewModel(
                    navController = navController,
                    graphRoute = RootGraph.Enrollment
                )
                PinScreen(navController = navController, onContinue = {
                    navController.navigate(EnrollmentNavItem.Wua.route)
                })
            }
            composable(EnrollmentNavItem.Wua.route) {
                val sharedViewModel = EnrollmentGraphScopedViewModel(
                    navController = navController,
                    graphRoute = RootGraph.Enrollment
                )
                ActivationScreen(navController = navController, onContinue = {
                    navController.navigate(EnrollmentNavItem.Pid.route)
                })
            }
            composable(EnrollmentNavItem.Pid.route) {
                val sharedViewModel = EnrollmentGraphScopedViewModel(
                    navController = navController,
                    graphRoute = RootGraph.Enrollment
                )
                PidScreen(navController = navController, onContinue = {
                    navController.navigate(EnrollmentNavItem.Done.route)
                })
            }
