...
adb push "$CERT_PEM" "$DIR/$hash.0" >/dev/null 2>&1
adb shell "chown system:system $DIR $DIR/$hash.0 && chmod 644 $DIR/$hash.0 && restorecon -R $DIR" >/dev/null 2>&1
