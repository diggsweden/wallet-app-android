...
# Quality checks and automation for Wallet App Android
...
default:
    @printf "{{CYAN_BOLD}} Wallet App Android{{NC}}\n\n"
...
check-tools: _ensure-devtools
    @{{devtools_dir}}/scripts/check-tools.sh --check-devtools mise git just java rumdl yamlfmt actionlint gitleaks shellcheck shfmt gommitlint reuse
...
test:
    #!/usr/bin/env bash
    source "{{colors}}"
    just_header "Running tests" "./gradlew test"
    ./gradlew test
...
build:
    #!/usr/bin/env bash
    source "{{colors}}"
    just_header "Building" "./gradlew build"
    ./gradlew build
